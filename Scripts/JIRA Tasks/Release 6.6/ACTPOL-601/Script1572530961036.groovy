...
if (Vreemdelingverify == verify2) {
    ...
    WebUI.selectOptionByLabel(findTestObject('Actpol_Scripts/Generiek/Assistentie/A Vreemdeling/Soort Document'), 'Visum', 
        false)

    WebUI.delay(2)

    String Number = WebUI.getNumberOfTotalOption(findTestObject('JIRA/A Relese 6.6/ACTPOL-309/Type'))

    KeywordUtil.logInfo(Number)

    String Number2 = WebUI.selectAllOption(findTestObject('JIRA/A Relese 6.6/ACTPOL-309/Type'))

    KeywordUtil.logInfo(Number2)

    String Number3 = WebUI.getText(findTestObject('JIRA/A Relese 6.6/ACTPOL-309/Type'))

    KeywordUtil.logInfo(Number3)

    String newdata = Number3

    println(newdata)

    WebUI.verifyEqual(newdata, newdata)
	
	KeywordUtil.markPassed('Dropdown values Type reisdocument is changed')
